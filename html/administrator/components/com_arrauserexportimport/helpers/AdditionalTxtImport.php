...
class TxtImport{
	...
	function getIDUserType($usertype){
		...
		$sql = "SELECT id
				FROM #__core_acl_aro_groups
				WHERE name = '" . addslashes($usertype) . "' limit 1";
		$db->setQuery($sql);
		$id .= $db->loadResult();
		return $id;
	}
	...
	function saveUsers($columns_imported, $row, $usertype, $gid, $columns_type){	   
	    $sql = "";
		$position = "";
		$id = "";
		$db =& JFactory::getDBO();
		...
		for($i=1; $i<=$number_of_columns; $i++){
			$add_column_name = JRequest::getVar("column".$i);
			if(trim($add_column_name) != ""){
				$sql .= ", `".addslashes(trim($add_column_name))."`";
				$default_columns_array[trim($add_column_name)] = trim(JRequest::getVar("default_value".$i));
			}
		}
		$sql .= ") values (";		
		...
		foreach($default_columns_array as $key=>$value){			
			if(in_array($key, $columns_imported)){
				$temp = array_keys($columns_imported, $key);
				$position = $temp[0];	
				if(trim($row[$position]) == ""){
					$sql .=  "'".addslashes(trim($value))."', ";
				}
				else{
					$sql .= "'".addslashes(trim($row[$position]))."', ";
				}		 
	...
	function updateUser($columns_imported, $row, $user_id_from_database, $columns_type){
			$db =& JFactory::getDBO();
			$changes_array = array();
			...
			if(in_array("usertype", $columns_imported)){
				...
				if($change_usertype_or_gid == false){
					...
					if($change_all == "no"){
						...
						$temp = array_keys($columns_imported, "usertype");
						$position = $temp[0];							 
						$sql_2 = "select id from #__core_acl_aro_groups where name='".trim($row[$position])."'";
						$sql_3 = "select id from #__core_acl_aro where value=".$user_id_from_database;
						$db->setQuery($sql_2);
						$gid = $db->loadResult();
						//if exist usertype from imported file
			...
			if(in_array("params", $columns_imported)){
				if($change_all != "no"){
					$temp = array_keys($columns_imported, "params");
					$position = $temp[0];
					$changes_array[] = " params='".trim($row[$position])."' ";
				}	  
			}
			...
			foreach($default_columns_array as $key=>$value){						
				if(in_array($key, $columns_imported)){
					if($change_all == "no"){						
						$change_column = JRequest::getVar("overwrite_".$key, "1", "post", "string");
						if(isset($change_column) && $change_column == 0){
							$temp = array_keys($columns_imported, $key);
							$position = $temp[0];
							if(trim($row[$position]) != ""){
								$changes_array[] = "`".$key."`='".addslashes(trim($row[$position]))."'";
							}
							elseif(trim($value) != ""){
								$changes_array[] = "`".$key."`='".addslashes(trim($value))."'";
							}
							else{
								$error_row = true;
							}
