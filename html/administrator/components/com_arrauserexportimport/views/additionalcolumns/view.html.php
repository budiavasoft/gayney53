 ...
 *
 **** class 
     ArrausersexportimportViewAdditionalcolumns 
...
/**
 * ArrausersexportimportViewImport View
 *
 */
class ArrausersexportimportViewAdditionalcolumns extends JView{
	/**
	 * display method 
	 * @return void
	 **/
	function display($tpl = null){						
		...
		$additional_columns = $this->additionalColumns();
		$this->assignRef('additional_columns', $additional_columns);
	...
	function additionalColumns(){
		...
		$return .= 	  "<tr>";
		$return .= 	  	  "<td class=\"td_settings_options\" width=\"80%\">";
		$return .= 	  	  		"<span class=\"editlinktip hasTip\" title=\"Additional Columns::".JText::_("ARRA_TIP_ADDITIONAL_COLUMNS_DETAILS") ."\" >".
											JText::_("ARRA_ADDITIONAL_COLUMNS_DETAILS");
		$return .= 	  	  		"</span>";
		$return .= 	  	  "</td>";
		$return .= 	  	  "<td align=\"center\">";
		$return .= 	  	  	'<select name="number_columns" onchange="javascript:setAdditionalColumn(this);">';
		for($i=0; $i<=$number; $i++){
		...
		for($i=0; $i<35; $i++){			
			$return .= 	  	 '<tr>';
			$return .= 	  	  	'<td>';
			$return .= 	  	  		'<div id="column'.($i+1).'" style="display:none; background-color:#F6F6F6; border-bottom:1px solid #E9E9E9; border-right:1px solid #E9E9E9; color:#666666; font-weight:bold; padding:5px;">';
			$return .= 	  	  		"<span class=\"editlinktip hasTip\" title=\"Additional Columns::".JText::_("ARRA_TIP_COLUMNS_DETAILS") ."\" >".JText::_("ARRA_COLUMN")." ".($i+1)."</span> &nbsp;".'<input type="text" name="column'.($i+1).'" />&nbsp;&nbsp;&nbsp; <span class="editlinktip hasTip" title="Additional Columns::'.JText::_("ARRA_TIP_DEFAULT_VALUE_DETAILS").'" >'.JText::_("ARRA_COLUMN_DEFAULT_VALUE")."</span>&nbsp;".'<input type="text" name="default_value'.($i+1).'" />';
			$return .= 	  	  		'<div>';
			$return .= 	  	  	'</td>';
			$return .= 	  	 '</tr>';
		}
	...
	function emailSettings(){
	    ...
	    if(strlen($all_result) != 0){
			...
			if(isset($all_array["AdditionalColumns"]) && strlen(trim($all_array["AdditionalColumns"])) != 0){								
			   $result = $all_array["AdditionalColumns"];			
		       $settings_saved = true;  
			}
			else{
			   $settings_saved = false;  
	...
	function uploadCsvTxtFile(){
		...
		$upload_file .= "<div class=\"space\"></div>";
		$upload_file .= 			    JText::_("ARRA_IMPORT_TXT_EXAMPLE")."&nbsp;&nbsp;&nbsp;&nbsp;"."<a href=\"".Juri::base()."components/com_arrauserexportimport/files/txt_example.txt"."\" target=\"_blank\"> example.txt</a>"; 
		$upload_file .= "<br/>";
		$upload_file .= 			    JText::_("ARRA_IMPORT_CSV_EXAMPLE")."&nbsp;&nbsp;"."<a href=\"".Juri::base()."components/com_arrauserexportimport/files/csv_example.csv"."\"> example.csv</a>"; 
		$upload_file .= 			"</td>";
		$upload_file .= 		"</tr>";
		$upload_file .= 		"<tr>";
		$upload_file .= 		   "<td>";
		$upload_file .= 		       "<a rel=\"{handler: 'iframe', size: {x: 500, y: 315}}\"  class=\"modal\"  href=\"index.php?option=com_arrauserexportimport&controller=modal&task=modal&tmpl=component\">".JText::_('VIEWPACKAGE_ALL_COLUMNS_BUTTON')."</a>";
		$upload_file .= 		   "</td>";
		$upload_file .= 		"</tr>";
		$upload_file .= "</table>";
	...
	function allSettings(){
		...
		$allSettings .=        "<span class=\"editlinktip hasTip\" title=\"" . JText::_("ARRA_SEPARATOR")."::".JText::_("ARRA_TIP_SEPARATOR") ."\" >".
						       ...
						       "</span>";
		$allSettings .=        "<span><a style=\"color:red;\" href=\"#\" onclick=\"javascript:hide_show('separator_div'); return false;\">".JText::_("ARRA_SEPARATOR_2_HEADER")."</a></span><div id=\"separator_div\" style=\"display:none; color:red;\">".JText::_("ARRA_SEPARATOR_22")."</div>";					   
		$allSettings .=         "</td>";
		...
		$additional_clumns = $this->get("AdditionalColumns");
		foreach($additional_clumns as $key=>$value){
			$allSettings .=     "<tr>";
			$allSettings .=         "<td class=\"td_settings_options\">";
			$allSettings .=        "<span class=\"editlinktip hasTip\" title=\"".JText::_("ARRA_OVERWRITE_ADDITIONAL_COLUMN")."::".JText::_("ARRA_TIP_OVERWRITE_ADDITIONAL_COLUMN")."\" >".
										JText::_("ARRA_OVERWRITE_ADDITIONAL_COLUMN")." - \"".$value."\"";
								   "</span>";
			$allSettings .=         "</td>";
			$allSettings .=         "<td class=\"td_settings\">";		
			$allSettings .=        	    JText::_("ARRA_YES") . "<input type=\"radio\" name=\"overwrite_".$value."\" value=\"0\" > ";
			$allSettings .=         	JText::_("ARRA_NO") . "<input type=\"radio\" name=\"overwrite_".$value."\" value=\"1\" checked> ";
			$allSettings .=         "</td>";
		}
	...
	function setDefaultUsertype(){
		...
		if(isset($all_user_type) && is_array($all_user_type) && count($all_user_type)!=0){
		    $encripted .=  		"<input type=\"text\" value=\"".$all_user_type[0]['name']."\" name=\"position\" class=\"combobox\" id=\"position\" style=\"position: absolute; top: 705px !important; left: 1005px !important; width: 58px; z-index: 1000; height: 12px;\">";
			...
			$encripted .=  "<input type=\"text\" name=\"position\" class=\"combobox\" id=\"position\" style=\"position: absolute; top: 705px; left: 1005px; width: 58px; z-index: 1000; height: 12px;\">";	
		}
		$encripted .=      "</td>";
		$encripted .=  "</tr>";
	...
	function getFields(){
		$return = '<select name="fields" id="fields">';
