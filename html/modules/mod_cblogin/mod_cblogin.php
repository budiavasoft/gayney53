...
$return		=	'http' . ( $isHttps ? 's' : '' ) . '://' . isset($_SERVER['HTTP_HOST']);
...
if ( $_CB_framework->myId() ) {
	...
	if ($show_avatar == 0) {
		if ($greeting) echo '<div align="right" style="color:#007053; float:right; margin-right:30px; width:400px;"><span id="mod_login_greeting'.$id_sfx.'">'.$cbUser->replaceUserVars(sprintf( _UE_HI_NAME, $name )).'</span>&nbsp; &nbsp;'.($horizontal ? "&nbsp;" : "");
	} else {
	...
	echo '<input type="submit" name="Submit" class="button'.$class_sfx.'" value="' . $buttonValue . '"' . $buttonStyle . ' style="background:none repeat scroll 0 0 #4B7B6F; border:1px solid #CCCCCC; margin:15px 0 10px; width:60px; color:#FFFFFF; text-align:center;" />';
	echo '</span>';
...
*/
	...
	echo '<div class="form-login">
           <div style="padding:30px 0; float:left;">
        	<h1>Login Area</h1>
           	<img src="/templates/gaineyrancha/images/lock.png" alt="GaineyRancha" style="float:left; width:78px; margin:0px 15px 0 0;" />
            <div class="form">';	//added for gainey
	...
	if ( $urlRegister ) {
		...
		//added for Gainey AJ 2012-07-24
		echo '<a href="mailto:grca@gaineyranchca.com" class="mod_login'.$class_sfx.'"> Contact Support </a>';
